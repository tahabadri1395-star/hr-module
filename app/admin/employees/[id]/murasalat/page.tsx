  ...
  return (
    <div>
      <div className="rounded-xl px-5 py-4 mb-6 inline-flex items-center gap-3" style={glassCard}>
        <span className="text-xl font-bold" style={{ color: unread > 0 ? "#F87171" : "#4ADE80" }}>{unread}</span>
        <span className="text-xs" style={{ color: muted }}>unread of {items.length} circulars visible to this Khidmat Guzar</span>
      </div>

      <div className="rounded-xl overflow-hidden" style={glassCard}>
        <div className="px-6 py-4 border-b" style={{ borderColor: "rgba(255,255,255,0.08)" }}>
          <h2 className="text-sm font-semibold" style={{ color: ink }}>Murasalat</h2>
        </div>
        {items.length === 0 ? (
          <div className="py-12 text-center text-sm" style={{ color: mutedFaint }}>No circulars apply to this Khidmat Guzar.</div>
        ) : (
            ...
            {items.map(m => (
              <div key={m.id} className="px-6 py-3.5 flex items-start justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <p className="text-xs mt-1" style={{ color: mutedFaint }}>{fmt(m.created_at)}</p>
                </div>
                <span className="text-xs font-semibold px-2.5 py-1 rounded-full shrink-0"
                  style={{ backgroundColor: m.is_read ? "rgba(74,222,128,0.15)" : "rgba(248,113,113,0.15)", color: m.is_read ? "#4ADE80" : "#F87171" }}>
