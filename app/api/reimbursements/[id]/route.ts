...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const res = await query(`SELECT * FROM hr_reimbursements WHERE id=$1 AND employee_id=$2`, [parseInt(id), employee.id]);
  if (!res.rows[0]) return NextResponse.json({ error: "Not found." }, { status: 404 });
  if (res.rows[0].status !== "pending") return NextResponse.json({ error: "Only pending claims can be withdrawn." }, { status: 400 });

  await query(`DELETE FROM hr_reimbursements WHERE id=$1`, [parseInt(id)]);
